...
- **Insight**: Late-stage probes continue to support an early convergence call. High-ucb exploration confirmed the function is effectively flat, so the strategy transitions into lock mode early.
...
- **Insight**: The ridge is fully mapped and now effectively locked. After baseline mapping, ei consistently refines the same promising ridge.
...
- **Insight**: Slow progress continues as the search narrows onto a viable pocket. Continued exploration is necessary because every gain comes from shaving away negative loss rather than finding a strong positive basin.
...
- **Insight**: Exploration budget is reduced as the feasible region tightens. The main improvement comes from using svm filtering to avoid obviously poor placements while still preserving a modest exploration budget.
...
- **Insight**: The posterior variance is near zero around the winning recipe. A week-2 switch into ei pays off immediately and the week-3 peak becomes the global anchor for the rest of the project.
...
- **Insight**: Progress becomes more exploitative as candidate quality improves. The model needs both higher noise tolerance and candidate filtering to keep making small but reliable gains.
...
- **Insight**: Week 8 likely reaches the long-run optimum. Moderate ucb keeps improving the incumbent until the landscape stabilizes around week 8.
...
- **Insight**: A lightweight NN+GP ensemble helps capture nonlinear interactions. Pure gp stabilizes the search after the weak week-1 ensemble, then a lightweight nn+gp blend helps squeeze out late-stage gains.

---

## Cross-Function Patterns and Technique Effectiveness

- **Phase pattern**: Weeks 1-3 emphasize exploration, weeks 4-7 use a more balanced policy, and weeks 8-13 increasingly lock onto incumbents. Week 8 fits that pattern through an exploitation-oriented allocation.
